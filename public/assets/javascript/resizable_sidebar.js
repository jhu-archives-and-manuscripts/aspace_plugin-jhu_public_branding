...
ResizableSidebar.prototype.bind_events = function() {
    ...
    $(document).on('mousemove', function (e) {
        ...
        var content_min_width = 300;

        // if it's on the right side
        var right_offset = (self.$row.width() + self.$row.offset().left) - cursor_x;

        right_offset = Math.max(right_offset, sidebar_min_width);
        var new_content_width = Math.max(self.$row.width() - right_offset, content_min_width);
